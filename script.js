...
// Function to extract team abbreviation from team string
function getTeamAbbreviation(teamString) {
  const matches = teamString.match(/\(([^)]+)\)/);
  return matches ? matches[1] : teamString;
}
...
function populateTeamFilter() {
  const teams = [...new Set(data.map(player => {
    const abbr = getTeamAbbreviation(player.team);
    return abbr;
  }))].sort();
...
function sortPlayers(players, column) {
  ...
  sortedPlayers.sort((a, b) => {
    ...
    if (column === 'team') {
      comparison = getTeamAbbreviation(a[column]).localeCompare(getTeamAbbreviation(b[column]));
    } else if (column === 'name') {
...
function updateTable() {
  ...
  if (currentSort.column) {
    filteredPlayers = sortPlayers(filteredPlayers, currentSort.column);
  }
...
tableHeaders.forEach((header, index) => {
  const columns = ['name', 'team', 'points', 'rebounds', 'assists'];
  header.addEventListener("click", () => {
    const column = columns[index];
    let filteredPlayers = filterPlayers();
    filteredPlayers = sortPlayers(filteredPlayers, column);
    populateTable(filteredPlayers);
  });
});

// Initialize the table and filter options
populateTeamFilter();
populateTable(data);
